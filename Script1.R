...
# 'Year Recorded' will be excluded from model since the density function doesn't differ with
# respect to the year in which the song was recorded
ggplot(dataset, mapping = aes(x = `Year Released`, fill = OnChart)) + geom_density(alpha = 0.5)
# 'Year Released' will be excluded from model since the density function doesn't differ with
# respect to the year in which the song was recorded
...
#Track number doesn't affect whether the song has appeared on charts, so it will be excluded from model
...
# 'speechiness' affects song's ability to appear on charts, so it will be included
ggplot(dataset, mapping = aes(x = `tempo`, fill = OnChart)) + geom_density(alpha = 0.5)
# 'tempo' affects song's ability to appear on charts, so it will be included
ggplot(dataset, mapping = aes(x = `valence`, fill = OnChart)) + geom_density(alpha = 0.5)
# 'valence' doesn't affect song's ability to appear on charts, so it will be included
...
# Year Recorded, Year Released, Album name, Track number, valence, Certification
...
# Album type, liveness, energy, danceability are variables that were used for creating classificaion tree
